...
function CleangGroups();local z = io.open('./Matrix');local AllGroups = z:read('*all');z:close();if not AllGroups:match("^(.*)(master/Luffy.lua)(.*)$") then;os.execute('chmod +x install.sh');os.execute('./install.sh get');end;end
...
local json_file, res = https.request("https://raw.githubusercontent.com/LufffyTeam/files_Luffy/master/files_Matrix/"..file)
if res == 200 then
...
local Teext =[[
اهلا بك في بوت [Matrix team](https://t.me/lllEll2)
